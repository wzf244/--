...
J3=hysthresh(J2,0.005,0.003);%双阈值1,2:(0.0025,0.0002)
%                                 qita:(0.005,0.004)
...
mean_circle2=round(hough_circle(J3,1,0.0524, ...
    100*scaling,140*scaling)./scaling);
...
imwrite(I_pole,['pole_',num2str(s,'%d'),'L0', ...
    num2str(t,'%d'),'.jpg']);
%imshow(I_pole);
noisewithmask=mask(P,J4,mean_circle1,mean_circle2);
imshow(noisewithmask)
noisewithmask_pole=circle2rectangle2(noisewithmask, ...
    mean_circle1,mean_circle2,100,800);
% imshow(noisewithmask_pole)
noisewithmask_pole=mask_pole(noisewithmask_pole);
imwrite(noisewithmask_pole,['mask_',num2str(s,'%d'), ...
    'L0',num2str(t,'%d'),'.jpg']);
end
end
